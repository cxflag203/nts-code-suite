  ...
  begin
   if not BufferedPaintRenderAnimation(Handle,PaintDC) then
    begin
     ...
     AnimationBuffer:= BeginBufferedAnimation(Handle, PaintDC, rtClient, BPBF_COMPOSITED, @PaintParams, AnimationParams, hdcFrom, hdcTo);
...
begin
 ...
 if ThemeServices.ThemesEnabled then
  begin
   CurrentAniState:= NewAniState;
   RConfig:= GetRenderState;
   if (arsGDIP in RConfig) then GPSurface:= TGPGraphics.Create(ACanvas.Handle);
